...
using BasicBot.Dialogs.Clothing;
...
namespace Microsoft.BotBuilderSamples
{
    ...
    public class BasicBot : IBot
    {
        ...
        private readonly IStatePropertyAccessor<ClothingState> _clothingStateAccessor;
        ...
        public async Task OnTurnAsync(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message)
            {
                ...
                productState = await _productStateAccessor.GetAsync(turnContext, () => new ProductState());
        ...
        private async Task<bool> IsTurnInterruptedAsync(DialogContext dc, string topIntent)
        {
            ...
            if (topIntent.Equals(ProductIntent))
            {
                await dc.CancelAllDialogsAsync();
                await dc.BeginDialogAsync(nameof(ShoesDialog));
                return true;
        ...
        private async Task UpdateProductState(RecognizerResult luisResult, ITurnContext turnContext)
        {
            if (luisResult.Entities != null && luisResult.Entities.HasValues)
            {
                ...
                productState.PriceMax = 0;
                productState.PriceMin = 0;

                // Supported LUIS Entities
                //string[] productEntities = { "product", "product_patternAny" };
                string[] categorieEntities = { "categorie", "categorie_patternAny" };
                string[] priceMinEntities = { "priceMin", "priceMin_patternAny" };
                string[] priceMaxEntities = { "priceMax", "priceMax_patternAny" };

                // Update any entities
                // Note: Consider a confirm dialog, instead of just updating.
                foreach (var categorie in categorieEntities)
                {
                    // Check if we found valid slot values in entities returned from LUIS.
                    if (entities[categorie] != null)
                    {
                        // Capitalize and set new user name.
                        var newCategorie = (string)entities[categorie][0];
                        productState.Categorie = char.ToUpper(newCategorie[0]) + newCategorie.Substring(1);
                        break;
                    }
                }
                ...
                //    {
                //        // Capitalize and set new user name.
                //        var newProduct = (string)entities[product][0];
                //        productState.Product = char.ToUpper(newProduct[0]) + newProduct.Substring(1);
                //        break;
                //    }
                //}
                ...
                foreach (var priceMin in priceMinEntities)
                {
                    if (entities[priceMin] != null)
                    {
                        ...
                        productState.PriceMin = Convert.ToDouble(newPriceMin);
                        break;
                    }
                }
                ...
                foreach (var priceMax in priceMaxEntities)
                {
                    if (entities[priceMax] != null)
                    {
                        ...
                        productState.PriceMax = Convert.ToDouble(newPriceMax);
                        break;
                    }
                }
                // Set the new values into state.
